"""Parse JSON responses from the Motorola OTA server into model dataclasses.

Every function takes a raw ``dict`` (already deserialised from JSON) and
returns the corresponding model.  Field mapping follows the exact
camelCase keys observed in captured server responses.
"""
...
def parse_check_response(data: Dict[str, Any]) -> CheckResponse:
    ...
    resp = CheckResponse(
        ...
        reporting_tags=data.get("reportingTags", ""),
