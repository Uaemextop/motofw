...
Downloads OTA packages from the URLs provided in server responses,
writing them to disk with :mod:`pathlib` and verifying the file hash
with :mod:`hashlib`.
"""
...
def download_update(
    ...
    *,
    ...
    response:
        A :class:`CheckResponse` with ``proceed=True`` and at least one
        content resource URL.
    client:
        Optional shared :class:`OTASession`.
    output_dir:
    ...
    if own_client:
        client = OTASession(cfg)

    try:
        resp = client.stream_get(download_url)  # type: ignore[union-attr]
        try:
            ...
            with dest_path.open("wb") as fh:
                for chunk in resp.iter_bytes(chunk_size=65536):
                    ...
                    if total:
                        ...
                        logger.debug(
                            "Progress: %.1f%% (%d / %d)", pct, downloaded, total
                        )
        finally:
            resp.close()
